...
export function getClusteredLighting(maxLightIndices = 8192) {
    return `
...
vec3 calculatePointLight(ClusteredLight light, vec3 worldPos, vec3 normal, vec3 viewDir, vec3 albedo) {
...
vec3 calculateSpotLight(ClusteredLight light, vec3 worldPos, vec3 normal, vec3 viewDir, vec3 albedo) {
    vec3 toLight = light.position - worldPos;
...
vec3 evaluateClusteredLights(vec3 worldPos, vec3 viewPos, vec3 normal, vec3 albedo) {
    ...
    vec4 clusterData = texture(clusterDataTexture, vec2((float(clusterIndex) + 0.5) / clusterTexWidth, 0.5));
